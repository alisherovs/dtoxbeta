...
async def ask_options(message: types.Message, state: FSMContext):
    ...
    await message.answer("🔠 <b>Variantlarni vergul bilan ajratib yozing:</b>\n<i>Masalan: Ha, Yo'q, Bilmayman</i>", parse_mode="HTML")
...
async def select_course_process(call: CallbackQuery, state: FSMContext):
    ...
    if not days:
        ...
        await call.message.edit_text(
            ...
            "Bu kurs necha kundan iborat? (Raqam yozing)", parse_mode="HTML"
        )
        await state.set_state(AdminState.setting_course_days)
    else:
        await call.message.edit_text(f"🗓 <b>{course_code}</b> ({days} kun). Kunni tanlang:", reply_markup=days_grid_kb(course_code, days), parse_mode="HTML")
...
async def save_course_days_handler(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"✅ <b>{data['course_code']}</b>: {days} kun belgilandi.", reply_markup=back_kb(), parse_mode="HTML")
    await message.answer(f"🗓 <b>{data['course_code']}</b> kursi.", reply_markup=days_grid_kb(data['course_code'], days), parse_mode="HTML")
    await state.clear()
...
async def day_menu(call: CallbackQuery):
    ...
    await call.message.edit_text(f"⚙️ <b>{c} {d}-kun</b>\n📂 Fayllar: {len(items)} ta", reply_markup=day_actions_kb(c, d), parse_mode="HTML")
...
async def add_c(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await call.message.answer(f"📥 <b>{c} {d}-kun</b>. Media yuboring.\nTugatgach '✅ TUGATISH' bosing.", reply_markup=finish_upload_kb(), parse_mode="HTML")
...
async def search_process(message: types.Message, state: FSMContext):
    ...
    if not user: 
        return await message.answer("❌ <b>Topilmadi.</b> Qayta urinib ko'ring:", reply_markup=back_kb(), parse_mode="HTML")
...
async def manual_ask(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"📖 <b>Qo'llanma Linki:</b>\n{link}\n\nYangilash uchun yangi linkni yuboring:", reply_markup=back_kb(), parse_mode="HTML")
...
async def broadcast_msg(message: types.Message, state: FSMContext):
    ...
    elif "Tasdiqlan" in message.text: target = "PENDING_APPROVAL"
...
async def broadcast_send(message: types.Message, state: FSMContext, bot: Bot):
    ...
    msg_count = 0
    await message.answer(f"🚀 <b>Xabar yuborish boshlandi...</b> (Jami: {len(users_to_send)} ta)", parse_mode="HTML")
